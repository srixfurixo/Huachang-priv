...
const segs = [
  { label: 'Raw Materials', pct: 65, value: 5492, color: '#1890FF' },
  { label: 'Finished Goods', pct: 30, value: 2535, color: '#10B981' },
  { label: 'Trading Items', pct: 5, value: 423, color: '#F97316' },
];

const StockPie = ({ composition = [] }) => {
  const chartData = (composition && composition.length > 0)
    ? composition.map((c, i) => ({
        label: c.item_type || 'Category',
        value: Number(c.qty_mt || 0),
        color: i === 0 ? '#1890FF' : i === 1 ? '#10B981' : '#F97316',
      }))
    : segs;
